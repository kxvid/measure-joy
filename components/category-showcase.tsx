...
export function CategoryShowcase() {
  ...
  return (
    <section className="border-b border-border bg-secondary/55 py-14 lg:py-24">
      <div className="mx-auto max-w-[1400px] px-5 lg:px-8">
        ...
        <div className="grid items-center gap-8 lg:grid-cols-2 lg:gap-16">
          ...
          <ul className="order-2 lg:order-1">
            {categories.map((cat, i) => (
              <li key={cat.name} onMouseEnter={() => setActive(i)}>
                <Link
                  href={cat.href}
                  className="group flex items-center justify-between gap-4 border-b border-foreground/15 py-4 lg:py-5"
                >
                  <span className="flex items-baseline gap-3 lg:gap-5">
                    ...
                    <span
                      className={`font-display text-2xl lg:text-4xl font-extrabold uppercase tracking-[-0.035em] transition-all duration-300 ${
                        active === i ? "translate-x-2 text-brand" : "text-foreground/35 group-hover:text-foreground"
                      }`}
                    >
                      {cat.name}
                    </span>
          ...
          <div className="order-1 hidden lg:block lg:order-2">
            <Link href={categories[active].href} className="group relative block aspect-[4/3] overflow-hidden border border-foreground/15 bg-white shadow-[10px_10px_0_0_var(--foreground)]">
              <AnimatePresence mode="wait">
                ...
                >
                  <Image
                    ...
                    fill
                    sizes="50vw"
                    className="object-contain drop-shadow-[0_18px_18px_rgba(0,0,0,0.12)] transition-transform duration-500 group-hover:scale-[1.025]"
                  />
                </motion.div>
              </AnimatePresence>
              <span className="absolute bottom-4 left-4 flex items-center gap-2 bg-foreground px-3 py-2 font-mono text-[10px] font-semibold uppercase tracking-[0.12em] text-background">
